...
**범위: Annex A 한정.** 이 문서들은 Annex A 93개 통제만 다룹니다. ISMS 요구사항이 있는 본문 4장에서
10장, 곧 범위, 리더십, 위험 평가와 위험 처리, 6.1.3이 요구하는 적용가능성 명세서(Statement of
Applicability), 목표, 역량, 문서화된 정보, 운용, 성과 평가, 내부 심사, 경영 검토, 부적합과
시정조치는 여기서 다루지 않습니다. 각 문서 `관련 통제 및 속성` 절의 `ISO 27001 본문 연계` 줄은
본문을 가리키는 포인터이지 본문 요약이 아니며, 그 요구사항이 필요하면 라이선스를 갖춘 표준 원본을
보아야 합니다. 적용가능성 명세서는 조직이 93개 통제 각각에 대한 판단을 기록하는 문서이고 Annex A
심사는 대개 여기서 출발합니다. 이 자료집은 그 문서가 아니며 그것을 만들어 주지도 않습니다.
...
[../AGENTS.md](../AGENTS.md)와 아래의 언어 대응 관계를 보십시오.
...
모든 통제 문서는 상단 메타데이터 표, 순서가 같은 6개 섹션, 하단 출처와 한계 고지를 지킵니다.
...
둘 다 이 프로젝트가 직접 쓴 것입니다. 그래도 두 쪽은 같은 내용을 말해야 하고 **같은 커밋에서** 함께

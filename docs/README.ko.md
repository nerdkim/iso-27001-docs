...
자료집을 **사용하는 동안에는** 이 디렉터리를 불변으로 취급하십시오. `tools/check_corpus.py`는 여기서 읽기만 하고
`tools/build_index.py`도 생성되는 `INDEX.md` 외에는 쓰지 않습니다. 자료집으로 답하는 agent는 `docs/`
아래의 무엇도 만들거나 고치거나 지우면 안 됩니다. 파생 산출물은 사용하는 쪽 작업 공간에 둡니다.
...
바뀌어야 합니다. 한국어만 또는 영어만 고친 상태는 결함이며, 섹션 항목 수가 달라지면 CI가 막습니다. 대응 관계는 통제 번호로
잡힙니다.

...
이 디렉터리에는 AI agent가 `docs/` 자료집 전체를 다시 읽지 않고도 자료집을 **사용**할 수 있게 해
주는 파일이 있습니다. agent가 자료집을 정직하게 사용하도록 하는 운영 규약도 여기에 둡니다. 이 readme 한 쌍과
`catalog/controls.json`을 빼면 전부 `docs/`와 `catalog/controls.json`에서 생성됩니다.
`catalog/controls.json`은 수작업으로 관리하는 입력이며, 어떤 통제가 존재해야 하는지를 정의하는
정본입니다.
...
자료집을 사용하는 agent가 지켜야 하는 규칙입니다. 자료집을 사용하는 환경의 `CLAUDE.md` 또는 `AGENTS.md`에
반영하십시오.
이 규칙을 전달받은 내용에 적용해 부적합 후보를 보고하는 Claude Code skill은
[../skill/iso-27001-review/SKILL.md](../skill/iso-27001-review/SKILL.md)에 있습니다.
이 skill 디렉터리를 `~/.claude/skills/`에 symlink로 연결해 설치하고 `/iso-27001-review`로 호출합니다.
다른 프로젝트에서 사용하는 방법은 [../README.ko.md](../README.ko.md)를 참고하십시오.

1. **`docs/`는 읽기 전용입니다.** 자료집을 사용하는 동안 `docs/` 아래에서 아무것도 만들거나
   고치거나 지우지 않습니다. 파생 산출물은 자료집을 사용하는 쪽 작업 공간에 쓰고, 이 저장소로 되돌려 쓰지 않습니다.
2. **manifest 우선 라우팅.** `manifest.json`을 먼저 읽습니다. 관련 통제 번호와 `path`로 범위를 좁힌
   뒤 해당 문서만 읽습니다. `docs/` 전체를 grep하지 않습니다.
3. **모든 주장에 출처를 답니다.** 각 진술에 `docs/` 경로와 섹션명을
   `[출처: docs/ko/A.5-organizational/A.5.1.md > 주요 확인사항]` 형태로 붙입니다. 출처를 제시할 수
   없으면 단정하지 말고, 자료집이 다루지 않는 내용이라고 밝힙니다.
4. **자료집 범위를 벗어나지 않습니다.** 이 저장소의 근거는 186개 `.md` 문서, `manifest.json`,
   `index/*`, `catalog/controls.json`입니다. 모델의 일반 지식으로 통제 요구사항이나 수치, 기준값을
   단정하지 않습니다.
5. **저작권 경계를 지킵니다.** 이 자료집의 설명 본문은 원저작물이며 표준 원문이 아닙니다. 이를
   ISO/IEC 27001:2022의 규범 텍스트인 것처럼 제시하지 않고, 표준 문구를 복원해 빈틈을 메우지
   않습니다. 정본이 필요한 답이면 라이선스를 갖춘 표준 원본을 확인하도록 안내합니다.
6. **사람 승인 게이트.** 인증 준비 상태 판단, 최종 적합성 판정, 보완조치 완료 판정은 agent가 아니라
   사람이 결정합니다. 결과는 검토 대상 후보로 제시합니다.

## 라우팅 예시

"접근통제가 운영되고 있음을 무엇으로 증명하는가"라는 질문은 다음 순서로 처리합니다.

1. `manifest.json`에서 `name`이 접근통제와 관련된 통제를 찾습니다(A.5.15에서 A.5.18 구간과
   A.8.2에서 A.8.5 구간).
2. 그 문서들만 읽어 `증적자료`와 `부적합 사례` 섹션을 가져옵니다. 목록만 필요하면
